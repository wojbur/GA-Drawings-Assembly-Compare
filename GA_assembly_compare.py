...
with list of assembly drawings"""
...
class GaAssemblyCompare:
    def __init__(self, main):
        ...
        self.button_compare = tk.Button(self.frame3, text='Compare marks \u279d', command=self.compare_drawings, width=15)
        self.button_compare.grid(row=0, column=0, columnspan=2)

        # Define output text widget
        self.output_txt = st.ScrolledText(self.frame4, width=30, height = 30)
        self.output_txt.grid(row=1, column=0)
        ...
        # Define drop down menu to select assembly mark pattern
        self.pattern = None

        self.dropdown_label = tk.Label(self.frame3, text='FAB:', width=5)
        self.dropdown_label.grid(row=1, column=0)

        self.fab_options = ['','ALM', 'BSC', 'TSC', 'CSS']
        ...
        self.dropdown.grid(row=1, column=1, sticky=tk.EW)

        self.pattern_label = tk.Label(self.frame3)
        self.pattern_label.grid(row=2, column=0, columnspan=2)
    ...
    def pattern_changed(self, *args):
        if self.fab.get() == '':
            ...
            self.pattern = re.compile(r'^\d{4,5}[A-Z]{1,2}$')
        elif self.fab.get() == 'BSC':
            self.pattern_label['text'] = 'num pattern: 7B134'
            self.pattern = re.compile(r'^\d+[A-Z]{1,3}\d+$')
